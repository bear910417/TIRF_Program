...
def update_fig(key_events, show, next, previous, go, dtime, etime, clickData, mode, save, load, loadp, rupture, good, bad, coloc, select, scatter, smooth, strided, rescale, relayout, channel, chp_find_0, chp_find_1, confirm_reset, i, path, 
    ...
    bkps = find_chp(changed_id, fret_g, fret_b, rr, gg, gr, bb, bg, br, i, time, select_list_g, 
...
@app.callback(
    ...
    State('gmm_cov_type', 'value'),
    State('path','value'),
    )
def update_Hist(fit, save, binsize, n_comps, means, channels, cov_type, path):
    changed_id = [p['prop_id'] for p in callback_context.triggered][0]
    ...
    if ('gmm_fit' in changed_id or 'gmm_comps' in changed_id):
        init_means = []
        if channels == 'fret_g':
            FRET_list = fret_g
    ...
    if 'binsize' in changed_id:
        fig2.update_traces(xbins=dict(start=0,end=1,size = float(binsize)),selector=dict(name='Dwell_time'))
