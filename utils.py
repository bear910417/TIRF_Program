...
def uf(t, lag):
    return uniform_filter1d(t, size = lag, mode = 'nearest')

def update_trace(fig, relayout, i, scatter, fret_g, fret_b, rr, gg, gr, bb, bg, br, time, bkps, lag):
    mode_dict = {
        0 : 'lines',
        1 : 'markers'
    }
    ...
    if np.any(fret_b):
        ...
        fig.update_traces(x = uf_time_b, y = uf(fret_b[i], lag), mode = mode_dict[scatter], selector = dict(name='fret_b'))
        fig.update_traces(x = uf_time_b, y = uf(bb[i], lag), mode = mode_dict[scatter], selector = dict(name='bb'))
        fig.update_traces(x = uf_time_b, y = uf(bg[i], lag), mode = mode_dict[scatter], selector = dict(name='bg'))
        fig.update_traces(x = uf_time_b, y = uf(br[i], lag), mode = mode_dict[scatter], selector = dict(name='br'))
    ...
    if np.any(fret_g):
        ...
        fig.update_traces(x = uf_time_g, y = uf(fret_g[i], lag), mode = mode_dict[scatter], selector = dict(name='fret_g'))
        fig.update_traces(x = uf_time_g, y = uf(gg[i], lag), mode = mode_dict[scatter], selector = dict(name='gg'))
        fig.update_traces(x = uf_time_g, y = uf(gr[i], lag), mode = mode_dict[scatter], selector = dict(name='gr'))
    ...
    if np.any(rr):
        fig.update_traces(x = uf(time['r'], lag), y = uf(rr[i], lag), mode = mode_dict[scatter], selector = dict(name='rr'))
