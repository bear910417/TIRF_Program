...
class Processor:
    ...
    def load_data_npz(self):
        ...
        for trace in range(int(self.N_traces)):
            if np.any(raw_gg):
                avg_gg[trace]= uf(raw_gg[trace], size = lag, mode = 'nearest')
                avg_gr[trace]= uf(raw_gr[trace], size = lag, mode = 'nearest')

            if np.any(raw_rr):
                avg_rr[trace]= uf(raw_rr[trace], size = lag, mode = 'nearest')
               
            if np.any(raw_bb):
                avg_bb[trace]= uf(raw_bb[trace], size = lag, mode = 'nearest')
                avg_bg[trace]= uf(raw_bg[trace], size = lag, mode = 'nearest')
                avg_br[trace]= uf(raw_br[trace], size = lag, mode = 'nearest')
